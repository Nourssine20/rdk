...
public class SshConnection implements Connection {
    ...
    public SshConnection(final Credential creds, final String username, final String password, final String host,
	    final int port, final String privateKeyLocation) throws Exception {
	...
	if (defaultPassword == null) {
	    throw new Exception("Unable to find password for " + defaultUsername);
	}
	...
	this.hostName = host;
	LOGGER.info("SSH Host IP from Constructor " + host);
	...
	/*    if (!portFound) {
	    ...
	    if (!portFound) {
		customPort = AutomaticsPropertyUtility.getProperty(Constants.SSH_CUSTOM_PORT);
		if (CommonMethods.isNotNull(customPort)) {
		    try {
			this.portNumber = Integer.valueOf(customPort);
     ...
     * @throws Exception 
     */
    public SshConnection(String username, String password, String host, String privateKeyLocation) throws Exception {
     ...
     * @throws Exception 
     */
    public SshConnection(String username, String password, String host) throws Exception {
     ...
     * @throws Exception 
     */
    public SshConnection(String username, String host) throws Exception {
     ...
     * @throws Exception 
     */
    public SshConnection(String host) throws Exception {
     ...
     * @throws Exception 
     */
    public SshConnection(String host, ConnectionType connectionType) throws Exception {
	this(SshConnectionUtils.getSshDetails(host), null, null, host, 0, null);
	...
	try {
	   ...
	   LOGGER.error("Execption occured : "+e.getMessage());
	}
    }
     ...
     *
     */
    private void connectWithDefaultCredentials() throws Exception {
	try {
	    JSch shell = new JSch();
	    ...
	    session.connect((int) Constants.THIRTY_SECONDS);
	    if (!session.isConnected()) {
		// wait for 2 seconds to establish connection to remove server.
		Thread.sleep((int) Constants.TWO_SECONDS);
	    }
	} catch (Exception exe) {
	...
	if (command.contains("sshtoatom") || command.contains("sshtoqtn")) {
	    Thread.sleep(3 * Constants.ONE_SECOND);
	}
    }
	...
	try {
		...
		while (defaultInputStream.available() > 0) {
			...
			if (check.indexOf(Constants.WARNING_MESSAGE_FOR_NEW_CONNECTION) != -1
				|| check.indexOf(Constants.ALTERNATE_WARNING_MESSAGE_FOR_NEW_CONNECTION) != -1) {
			    String yesKeyword = check.contains(Constants.WARNING_MESSAGE_FOR_NEW_CONNECTION) ? "yes"
				    : "y";
	...
	return getSettopResponse(Constants.THIRTY_SECONDS);
    }
	...
	Thread responseReadThread = new Thread() {
		...
		try {
		    while (true) {
			// Getting actual response for command execution.
			String check = Constants.EMPTY_STRING;
			while (defaultInputStream.available() > 0) {
			    int noChars = defaultInputStream.read(tmp, 0, tmp.length);
				...
				if (check.indexOf(Constants.SUDO_PASS_WORD_PROMPT + defaultUsername
					+ Constants.COLON_WITH_SPACE) != -1) {
				    String passwordText = defaultPassword;
				...
				if (check.indexOf(Constants.WARNING_MESSAGE_FOR_NEW_CONNECTION) != -1
					|| check.indexOf(
						Constants.ALTERNATE_WARNING_MESSAGE_FOR_NEW_CONNECTION) != -1) {
				    String yesKeyword = check.contains(
					    Constants.WARNING_MESSAGE_FOR_NEW_CONNECTION) ? "yes" : "y";
				    byte[] passwd = (yesKeyword + "\n").getBytes();
				    defaultOutputStream.write(passwd);
				    defaultOutputStream.flush();
				}
	...
	return commandResponse.toString().replaceAll(Constants.ANSI_REGEX, Constants.EMPTY_STRING)
		.replaceAll(Constants.EMPTY_LINE_REMOVER_REGEX, Constants.EMPTY_STRING)
		.replaceAll("\\[\\[[;\\d]*m", "");
    }
	...
	channel.connect(Constants.THIRTY_SECONDS_INT);
	defaultInputStream = channel.getInputStream();
